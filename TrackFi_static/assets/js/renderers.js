const ICONS = {
  dashboard: `
    <svg viewBox="0 0 18 19" aria-hidden="true">
      <path d="M17.0002 14.3425V8.7945C17.0002 8.2605 17.0002 7.9935 16.9352 7.7445C16.8777 7.52441 16.783 7.31575 16.6552 7.1275C16.5102 6.9145 16.3102 6.7375 15.9072 6.3865L11.1072 2.1865C10.3612 1.5335 9.98724 1.2065 9.56724 1.0825C9.19724 0.9725 8.80324 0.9725 8.43224 1.0825C8.01224 1.2065 7.64024 1.5325 6.89424 2.1845L2.09324 6.3865C1.69124 6.7385 1.49024 6.9145 1.34624 7.1265C1.21804 7.31498 1.12301 7.52399 1.06524 7.7445C1.00024 7.9925 1.00024 8.2605 1.00024 8.7945V14.3425C1.00024 15.2745 1.00024 15.7405 1.15224 16.1075C1.25264 16.3502 1.39987 16.5708 1.58552 16.7566C1.77117 16.9424 1.99161 17.0899 2.23424 17.1905C2.60224 17.3425 3.06824 17.3425 4.00024 17.3425C4.93224 17.3425 5.39824 17.3425 5.76624 17.1905C6.00888 17.0899 6.22932 16.9424 6.41497 16.7566C6.60062 16.5708 6.74785 16.3502 6.84824 16.1075C7.00024 15.7405 7.00024 15.2745 7.00024 14.3425V13.3425C7.00024 12.8121 7.21096 12.3034 7.58603 11.9283C7.9611 11.5532 8.46981 11.3425 9.00024 11.3425C9.53068 11.3425 10.0394 11.5532 10.4145 11.9283C10.7895 12.3034 11.0002 12.8121 11.0002 13.3425V14.3425C11.0002 15.2745 11.0002 15.7405 11.1522 16.1075C11.2526 16.3502 11.3999 16.5708 11.5855 16.7566C11.7712 16.9424 11.9916 17.0899 12.2342 17.1905C12.6022 17.3425 13.0682 17.3425 14.0002 17.3425C14.9322 17.3425 15.3982 17.3425 15.7662 17.1905C16.0089 17.0899 16.2293 16.9424 16.415 16.7566C16.6006 16.5708 16.7478 16.3502 16.8482 16.1075C17.0002 15.7405 17.0002 15.2745 17.0002 14.3425Z" fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" stroke-width="2" />
    </svg>
  `,
  transactions: `
    <svg viewBox="0 0 20 16" aria-hidden="true">
      <path d="M1 7V11.8C1 12.92 1 13.48 1.218 13.908C1.40974 14.2843 1.71569 14.5903 2.092 14.782C2.519 15 3.079 15 4.197 15H15.803C16.921 15 17.48 15 17.907 14.782C18.284 14.59 18.59 14.284 18.782 13.908C19 13.48 19 12.922 19 11.804V7M1 7V5M1 7H19M19 7V5M1 5V4.2C1 3.08 1 2.52 1.218 2.092C1.41 1.715 1.715 1.41 2.092 1.218C2.52 1 3.08 1 4.2 1H15.8C16.92 1 17.48 1 17.907 1.218C18.284 1.41 18.59 1.715 18.782 2.092C19 2.519 19 3.079 19 4.197V5M1 5H19M5 11H9" fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" stroke-width="2" />
    </svg>
  `,
  categories: `
    <svg viewBox="0 0 20 20" aria-hidden="true">
      <path d="M10 1C8.21997 1 6.47991 1.52784 4.99987 2.51677C3.51983 3.50571 2.36628 4.91131 1.68509 6.55585C1.0039 8.20038 0.82567 10.01 1.17294 11.7558C1.5202 13.5016 2.37737 15.1053 3.63604 16.364C4.89472 17.6226 6.49836 18.4798 8.24419 18.8271C9.99002 19.1743 11.7996 18.9961 13.4442 18.3149C15.0887 17.6337 16.4943 16.4802 17.4832 15.0001C18.4722 13.5201 19 11.78 19 10M10 1C12.387 1 14.6761 1.94821 16.364 3.63604C18.0518 5.32387 19 7.61305 19 10M10 1V10M19 10H10M10 10L16 16.5" fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" stroke-width="2" />
    </svg>
  `,
  analytics: `
    <svg viewBox="0 0 20 20" aria-hidden="true">
      <path d="M3 15.5h14M4.5 12.5l4-4 2.9 2.9 4-5.1" fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" stroke-width="2.1" />
    </svg>
  `,
  goals: `
    <svg viewBox="0 0 20 20" aria-hidden="true">
      <path d="M5.5 10H10M10 10H14.5M10 10V14.5M10 10V5.5M1 15.4V4.6C1 3.34 1 2.71 1.24525 2.2285C1.46125 1.80438 1.80438 1.46125 2.2285 1.24525C2.71 1 3.34 1 4.6 1H15.4C16.66 1 17.29 1 17.7715 1.24525C18.1948 1.46096 18.539 1.80515 18.7547 2.2285C19 2.71 19 3.34 19 4.6V15.4C19 16.66 19 17.29 18.7547 17.7715C18.539 18.1948 18.1948 18.539 17.7715 18.7547C17.29 19 16.6623 19 15.4045 19H4.59663C3.33887 19 2.70887 19 2.2285 18.7547C1.80515 18.539 1.46096 18.1948 1.24525 17.7715C1 17.29 1 16.66 1 15.4Z" fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" stroke-width="2" />
    </svg>
  `,
  settings: `
    <svg viewBox="0 0 22 21" aria-hidden="true">
      <path d="M19.3464 7.3503L18.9804 7.1463L18.8674 7.08229C18.5948 6.91868 18.3651 6.69242 18.1974 6.4223C18.1794 6.3953 18.1634 6.3663 18.1314 6.3103C17.9158 5.96422 17.8111 5.56053 17.8314 5.1533L17.8374 4.7283C17.8494 4.0483 17.8554 3.7063 17.7594 3.4003C17.6745 3.12804 17.5323 2.87709 17.3424 2.6643C17.1284 2.4243 16.8314 2.2523 16.2364 1.9103L15.7424 1.6253C15.1504 1.2843 14.8534 1.1133 14.5384 1.0483C14.2601 0.990707 13.9727 0.993093 13.6954 1.0553C13.3824 1.1253 13.0894 1.3013 12.5044 1.6513L12.5014 1.6533L12.1474 1.8643C12.0914 1.8983 12.0624 1.9143 12.0344 1.9303C11.7564 2.0853 11.4464 2.1703 11.1274 2.1803C11.0954 2.1823 11.0624 2.1823 10.9974 2.1823L10.8674 2.1813C10.5483 2.17113 10.2363 2.08472 9.95742 1.9293C9.92942 1.9143 9.90242 1.8973 9.84642 1.8633L9.48942 1.6493C8.90042 1.2953 8.60542 1.1193 8.29042 1.0483C8.01207 0.985991 7.72362 0.983946 7.44442 1.0423C7.12842 1.1083 6.83242 1.2803 6.23942 1.6243L6.23642 1.6253L5.74842 1.9083L5.74342 1.9123C5.15542 2.2523 4.86042 2.4243 4.64842 2.6633C4.45949 2.87569 4.31802 3.12592 4.23342 3.3973C4.13842 3.7043 4.14342 4.0463 4.15542 4.7303L4.16242 5.1543C4.16242 5.2193 4.16542 5.2513 4.16442 5.2823C4.15924 5.64562 4.05518 6.00066 3.86342 6.3093C3.83042 6.3653 3.81542 6.3933 3.79842 6.4193C3.62981 6.69147 3.39832 6.91918 3.12342 7.0833L3.01142 7.1463L2.65042 7.34629C2.04842 7.67929 1.74742 7.84629 1.52942 8.08429C1.3356 8.29413 1.18898 8.54305 1.09942 8.8143C0.999418 9.1213 0.999418 9.4643 1.00042 10.1523L1.00242 10.7153C1.00342 11.3983 1.00542 11.7393 1.10642 12.0443C1.1956 12.3139 1.34116 12.5613 1.53342 12.7703C1.75142 13.0063 2.04942 13.1723 2.64642 13.5043L3.00442 13.7033C3.06542 13.7373 3.09642 13.7533 3.12542 13.7713C3.43879 13.9591 3.69472 14.2293 3.86542 14.5523L3.93242 14.6723C4.10133 14.9912 4.1808 15.3499 4.16242 15.7103L4.15542 16.1173C4.14342 16.8033 4.13842 17.1473 4.23442 17.4543C4.31942 17.7263 4.46142 17.9773 4.65142 18.1903C4.86542 18.4303 5.16342 18.6013 5.75742 18.9443L6.25142 19.2293C6.84442 19.5703 7.14042 19.7413 7.45542 19.8063C7.73372 19.8639 8.02111 19.8615 8.29842 19.7993C8.61242 19.7293 8.90542 19.5533 9.49242 19.2013L9.84642 18.9893L9.95942 18.9233C10.2374 18.7693 10.5474 18.6833 10.8664 18.6733L10.9964 18.6723H11.1264C11.4444 18.6823 11.7564 18.7693 12.0364 18.9243L12.1284 18.9793L12.5044 19.2053C13.0944 19.5593 13.3884 19.7353 13.7034 19.8053C13.9816 19.8683 14.2701 19.871 14.5494 19.8133C14.8644 19.7473 15.1624 19.5743 15.7554 19.2303L16.2504 18.9433C16.8384 18.6013 17.1334 18.4303 17.3454 18.1913C17.5354 17.9783 17.6754 17.7283 17.7604 17.4573C17.8554 17.1523 17.8504 16.8133 17.8384 16.1393L17.8304 15.6993V15.5723C17.8351 15.2087 17.9388 14.8533 18.1304 14.5443L18.1954 14.4343C18.364 14.1621 18.5955 13.9344 18.8704 13.7703L18.9804 13.7093L18.9824 13.7083L19.3434 13.5083C19.9454 13.1743 20.2464 13.0083 20.4654 12.7703C20.6594 12.5603 20.8054 12.3103 20.8944 12.0403C20.9944 11.7353 20.9944 11.3933 20.9924 10.7133L20.9904 10.1393C20.9894 9.4563 20.9884 9.1143 20.8874 8.8093C20.7978 8.54 20.6519 8.29286 20.4594 8.08429C20.2424 7.84829 19.9444 7.68229 19.3484 7.35129L19.3464 7.3503Z" fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" stroke-width="2" />
      <path d="M6.99634 10.4273C6.99634 11.4882 7.41777 12.5056 8.16791 13.2557C8.91806 14.0059 9.93547 14.4273 10.9963 14.4273C12.0572 14.4273 13.0746 14.0059 13.8248 13.2557C14.5749 12.5056 14.9963 11.4882 14.9963 10.4273C14.9963 9.36643 14.5749 8.34901 13.8248 7.59886C13.0746 6.84872 12.0572 6.42729 10.9963 6.42729C9.93547 6.42729 8.91806 6.84872 8.16791 7.59886C7.41777 8.34901 6.99634 9.36643 6.99634 10.4273Z" fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" stroke-width="2" />
    </svg>
  `,
};

const HTML_ESCAPE_MAP = {
  "&": "&amp;",
  "<": "&lt;",
  ">": "&gt;",
  '"': "&quot;",
  "'": "&#39;",
};

function escapeHtml(value) {
  return String(value).replace(/[&<>"']/g, (character) => HTML_ESCAPE_MAP[character]);
}

function formatCurrency(value) {
  const hasCents = !Number.isInteger(value);
  return new Intl.NumberFormat("en-US", {
    style: "currency",
    currency: "USD",
    minimumFractionDigits: hasCents ? 2 : 0,
    maximumFractionDigits: 2,
  }).format(value);
}

function formatSignedCurrency(value) {
  const sign = value >= 0 ? "+" : "-";
  return `${sign}${formatCurrency(Math.abs(value))}`;
}

function renderIcon(name) {
  return ICONS[name] ?? "";
}

function renderSidebarList(items, extraClassName = "") {
  return `
    <ul class="sidebar__list ${extraClassName}">
      ${items
        .map(
          (item) => `
            <li class="sidebar__list-item">
              <a
                class="sidebar__item ${item.active ? "is-active" : ""}"
                href="${escapeHtml(item.href ?? `#${item.id}`)}"
                aria-label="${escapeHtml(item.label)}"
                ${item.active ? 'aria-current="page"' : ""}
              >
                <span class="sidebar__icon sidebar__icon--${escapeHtml(item.icon)}">${renderIcon(item.icon)}</span>
                <span class="sidebar__label">${escapeHtml(item.label)}</span>
              </a>
            </li>
          `,
        )
        .join("")}
    </ul>
  `;
}

export function renderSidebar(items) {
  const primaryItems = items.filter((item) => item.id !== "settings");
  const secondaryItems = items.filter((item) => item.id === "settings");

  return `
    <div class="sidebar__groups">
      ${renderSidebarList(primaryItems)}
      ${secondaryItems.length ? renderSidebarList(secondaryItems, "sidebar__list--secondary") : ""}
    </div>
  `;
}

export function renderStats(cards) {
  return cards
    .map(
      (card) => `
        <article class="stat-card stat-card--span-${card.columnSpan} stat-card--${card.tone}">
          <p class="stat-card__label">${escapeHtml(card.title)}</p>
          <p class="stat-card__value">${escapeHtml(card.valueText)}</p>
        </article>
      `,
    )
    .join("");
}

export function renderTransactions(items) {
  return items
    .map(
      (item) => `
        <article class="transaction">
          <div class="transaction__copy">
            <p class="transaction__title">${escapeHtml(item.title)}</p>
            ${
              item.category
                ? `<p class="transaction__category">${escapeHtml(item.category)}</p>`
                : ""
            }
          </div>
          <p class="transaction__amount ${item.amount >= 0 ? "is-income" : "is-expense"}">
            ${formatSignedCurrency(item.amount)}
          </p>
          <p class="transaction__date">${escapeHtml(item.date)}</p>
        </article>
      `,
    )
    .join("");
}

export function renderSpendingLegend(categories) {
  return categories
    .map(
      (category) => `
        <div class="spending-legend__item">
          <div class="spending-legend__label-group">
            <span class="spending-legend__swatch" style="background: ${category.color};"></span>
            <p class="spending-legend__label">${escapeHtml(category.label)}</p>
          </div>
          <p class="spending-legend__value">${formatCurrency(category.value)}</p>
        </div>
      `,
    )
    .join("");
}

export function renderBudgetGoals(goals) {
  return goals
    .map((goal) => {
      const progress = Math.min((goal.current / goal.target) * 100, 100);

      return `
        <article class="budget-goal">
          <div class="budget-goal__meta">
            <p class="budget-goal__name">${escapeHtml(goal.name)}</p>
            <p class="budget-goal__value">
              <span class="budget-goal__value-current">${formatCurrency(goal.current)}</span>
              <span class="budget-goal__value-separator"> / </span>
              <span class="budget-goal__value-target">${formatCurrency(goal.target)}</span>
            </p>
          </div>
          <div
            class="budget-goal__track"
            role="progressbar"
            aria-label="${escapeHtml(goal.name)} progress"
            aria-valuemin="0"
            aria-valuemax="100"
            aria-valuenow="${Math.round(progress)}"
          >
            <span
              class="budget-goal__bar"
              style="width: ${progress.toFixed(2)}%; background: ${goal.color};"
            ></span>
          </div>
        </article>
      `;
    })
    .join("");
}

export function renderUserMenu(menuItems) {
  return menuItems
    .map(
      (item) =>
        item.href
          ? `
            <a
              class="user-dropdown__menu-item"
              href="${escapeHtml(item.href)}"
              role="menuitem"
            >
              ${escapeHtml(item.label)}
            </a>
          `
          : `
            <button class="user-dropdown__menu-item" type="button" role="menuitem">
              ${escapeHtml(item.label)}
            </button>
          `,
    )
    .join("");
}

export function renderTransactionsSummary(summaryItems) {
  return summaryItems
    .map(
      (item) => `
        <article class="transactions-summary__item transactions-summary__item--${escapeHtml(item.tone)}">
          <p class="transactions-summary__label">${escapeHtml(item.label)}</p>
          <p class="transactions-summary__value">${escapeHtml(item.value)}</p>
        </article>
      `,
    )
    .join("");
}

export function renderTransactionsTable(columns, rows) {
  return `
    <table class="transactions-table">
      <colgroup>
        <col>
        <col>
        <col>
        <col>
        <col>
      </colgroup>
      <thead>
        <tr>
          ${columns
            .map(
              (column) => `
                <th class="transactions-table__head ${column.align === "right" ? "is-right" : ""}">
                  ${escapeHtml(column.label)}
                </th>
              `,
            )
            .join("")}
        </tr>
      </thead>
      <tbody>
        ${rows
          .map(
            (row) => `
              <tr class="transactions-table__row">
                <td class="transactions-table__cell transactions-table__cell--date">
                  ${escapeHtml(row.date)}
                </td>
                <td class="transactions-table__cell transactions-table__cell--type">
                  <span class="transaction-badge transaction-badge--${escapeHtml(row.tone)}">
                    ${escapeHtml(row.type)}
                  </span>
                </td>
                <td class="transactions-table__cell transactions-table__cell--category">
                  ${escapeHtml(row.category)}
                </td>
                <td class="transactions-table__cell transactions-table__cell--description">
                  ${escapeHtml(row.description)}
                </td>
                <td class="transactions-table__cell transactions-table__cell--amount ${row.tone === "income" ? "is-income" : "is-expense"}">
                  ${escapeHtml(row.amount)}
                </td>
              </tr>
            `,
          )
          .join("")}
      </tbody>
    </table>
  `;
}

export function renderCategoriesList(categories) {
  return `
    <div class="categories-list" role="list">
      ${categories
        .map(
          (category) => `
            <article class="category-row" role="listitem">
              <p class="category-row__name">${escapeHtml(category.name)}</p>
              <div class="category-row__actions" role="group" aria-label="${escapeHtml(category.name)} actions">
                <button class="category-action" type="button">
                  ${escapeHtml(category.editLabel)}
                </button>
                <button class="category-action category-action--delete" type="button">
                  ${escapeHtml(category.deleteLabel)}
                </button>
              </div>
            </article>
          `,
        )
        .join("")}
    </div>
  `;
}

export function renderGoalsManager(goals) {
  return `
    <div class="goals-manager" role="list">
      ${goals
        .map((goal) => {
          const progress = Math.min((goal.current / goal.target) * 100, 100);
          const currentText = goal.currentDisplay ?? formatCurrency(goal.current);
          const targetText = goal.targetDisplay ?? formatCurrency(goal.target);

          return `
            <article class="goal-row" role="listitem">
              <div class="goal-row__content">
                <div class="goal-row__meta">
                  <div class="goal-row__title-group">
                    <p class="goal-row__name">${escapeHtml(goal.name)}</p>
                    <p class="goal-row__label">${escapeHtml(goal.progressLabel)}</p>
                  </div>
                  <p class="goal-row__value">
                    <span class="goal-row__value-current">${escapeHtml(currentText)}</span>
                    <span class="goal-row__value-separator"> / </span>
                    <span class="goal-row__value-target">${escapeHtml(targetText)}</span>
                  </p>
                </div>
                <div
                  class="goal-row__track"
                  role="progressbar"
                  aria-label="${escapeHtml(goal.name)} progress"
                  aria-valuemin="0"
                  aria-valuemax="100"
                  aria-valuenow="${Math.round(progress)}"
                >
                  <span
                    class="goal-row__bar"
                    style="width: ${progress.toFixed(2)}%; background: ${goal.color};"
                  ></span>
                </div>
              </div>
              <div class="goal-row__actions" role="group" aria-label="${escapeHtml(goal.name)} actions">
                <button class="goal-row__contribute" type="button">${escapeHtml(goal.contributeLabel)}</button>
                <div class="goal-row__minor-actions">
                  <button class="category-action" type="button">${escapeHtml(goal.editLabel)}</button>
                  <button class="category-action category-action--delete" type="button">${escapeHtml(goal.deleteLabel)}</button>
                </div>
              </div>
            </article>
          `;
        })
        .join("")}
    </div>
  `;
}
